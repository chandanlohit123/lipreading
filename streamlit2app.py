...
if options: 
    ...
    with col2: 
        ...
        # Plot accuracy and precision graphs
        epochs = np.arange(1, 11)
        accuracy_values = np.random.uniform(70, 95, size=10)  # Example accuracy values
        precision_values = np.random.uniform(60, 90, size=10)  # Example precision values

        fig, ax = plt.subplots()
        ax.plot(epochs, accuracy_values, label='Accuracy', marker='o')
        ax.plot(epochs, precision_values, label='Precision', marker='o')
        ax.set_xlabel('Epochs')
        ax.set_ylabel('Percentage')
        ax.set_title('Accuracy and Precision Over Epochs')
        ax.legend()
        st.pyplot(fig)
